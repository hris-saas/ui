<template>
  <div class="flex-1 relative z-0 flex overflow-hidden">
    <main
      ...
      tabindex="0"
    >
      <article v-if="employee" ref="employee">
        <!-- Profile header -->
        <div>
          <div>
            <img
            ...
            >
              <div class="flex-shrink-0">
                <img
                  ...
                  :src="
                    'https://randomuser.me/api/portraits/' +
                    (employee.reports_to.gender === 'm' ? 'men' : 'women') +
                    ('/' + employee.reports_to.id + '.jpg')
                  "
                  alt=""
                />
              </div>
              <div class="flex-1 min-w-0">
                <a
                  ...
                  @click.prevent="navigateTo(employee.reports_to)"
                >
                  ...
                  <p class="text-sm text-gray-500 truncate">
                    {{ employee.reports_to.department.name[lang] }}
                  </p>
                </a>
              </div>
            </div>
        ...
        <!-- Team member list -->
        <div
          v-if="employee.direct_reports.length"
          class="max-w-5xl mx-auto px-4 pb-12 sm:px-6 lg:px-8"
        >
          <h2 class="text-sm font-medium text-gray-500">Direct reports</h2>
          <div class="mt-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div
              v-for="(directReport, key) in employee.direct_reports"
              :key="key"
              class="relative rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm flex items-center space-x-3 hover:border-gray-400 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500"
            >
              <div class="flex-shrink-0">
              ...
              <div class="flex-1 min-w-0">
                <a
                  ref="directReports"
                  class="focus:outline-none"
                  @click.prevent="navigateTo(directReport)"
                  href="#"
                >
                  <span class="absolute inset-0" aria-hidden="true"></span>
                  <p class="text-sm font-medium text-gray-900">
...
export default {
  ...
  head() {
    if (this.employee) {
      return { title: this.employee.first_name + ' ' + this.employee.last_name }
    }
  },
  ...
  async created() {
    await this.fetchEmployee({ id: this.$route.params.uuid })

    if (this.employee) {
      this.$emit('setParamName', this.employee)
    }
  },
  methods: {
    ...mapActions({
    ...
    async navigateTo(employee) {
      ...
      this.scrollTo(this.$refs.employee)
