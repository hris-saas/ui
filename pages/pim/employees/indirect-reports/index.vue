  ...
  <div class="max-w-8xl mx-auto px-4 sm:px-6 md:px-8">
    ...
    <div class="py-4">
      <div class="bg-white shadow overflow-hidden sm:rounded-md">
        <ul>
          <li v-for="(employee, key) in employeeIndirectReports" :key="key">
            <nuxt-link
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      employeeIndirectReports: 'employees/employeeIndirectReports'
    })
  },
  async created() {
    await this.fetchEmployee({ id: this.$route.params.id })
    await this.fetchEmployeeIndirectReports({ id: this.$route.params.id })
  },
  methods: {
    ...mapActions({
      fetchEmployee: 'employees/fetchEmployee',
      fetchEmployeeIndirectReports: 'employees/fetchEmployeeIndirectReports'
    }),
    paginate(page) {
      this.fetchEmployeeIndirectReports({ id: this.$route.params.id, page })
      this.overlay = true
    }
  },
  head() {
    return { title: this.$t('indirect_reports') }
  }
}
</script>
