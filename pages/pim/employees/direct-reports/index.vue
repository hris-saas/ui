...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      employeeDirectReports: 'employees/employeeDirectReports'
    })
  },
  async created() {
    await this.fetchEmployee({ id: this.$route.params.id })
    await this.fetchEmployeeDirectReports({ id: this.$route.params.id })
  },
  methods: {
    ...mapActions({
      fetchEmployee: 'employees/fetchEmployee',
      fetchEmployeeDirectReports: 'employees/fetchEmployeeDirectReports'
    }),
    paginate(page) {
      this.fetchEmployeeDirectReports({ id: this.$route.params.id, page })
      this.overlay = true
    }
  },
  head() {
    return { title: this.$t('direct_reports') }
  }
}
</script>
